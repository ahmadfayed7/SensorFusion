...
void clusterKptMatchesWithROI(BoundingBox &boundingBox, std::vector<cv::KeyPoint> &kptsPrev, std::vector<cv::KeyPoint> &kptsCurr, std::vector<cv::DMatch> &kptMatches)
{
	...
	const double distanceMean{ std::accumulate(distance.begin(), distance.end(), 0.0) / (distance.size()) };
                     ...
                     std::vector<LidarPoint> &lidarPointsCurr, double frameRate, double &TTC)
{
  ...
  bool matchPointToBoundingBox(const DataFrame &frame, const int index, int &matchingBoxId) {
	int matchCount=0 ;
     cv::Point2f point(frame.keypoints.at(index).pt);

    for (int currentBoundingBoxIndex{ 0 }; currentBoundingBoxIndex < frame.boundingBoxes.size(); currentBoundingBoxIndex++) {
        //check if point exist in box or not
        if (!frame.boundingBoxes.at(currentBoundingBoxIndex).roi.contains(point)) 
           continue; 
        //exist function if there is more than one match in same box to clasiify best match
        if (++matchCount > 1) {
            matchingBoxId = -1;
            return false;
        }
  ...
  int getMaximumIndex(const std::vector<int> &Data){
...
void matchBoundingBoxes(std::vector<cv::DMatch> &matches, std::map<int, int> &bbBestMatches, DataFrame &prevFrame, DataFrame &currFrame) {
	...
	const int columns =  prevFrame.boundingBoxes.size() ;
	const int rows =currFrame.boundingBoxes.size() ;
    // 2D vector to match bounding boxs
    std::vector<std::vector<int>> Matches(columns, std::vector<int>(rows, 0));

    int prevFrameMatchBoxId=-1, currFrameMatchBoxId=-1 ;
      //loop for all keypoint match
	for(auto match : matches){

        //map query point in prev frame to Bounding box
		if(!matchPointToBoundingBox(prevFrame, match.queryIdx, prevFrameMatchBoxId))  
           continue; 
        //map train  point in current frame to Bounding box
		if(!matchPointToBoundingBox(currFrame, match.trainIdx, currFrameMatchBoxId))
          continue;
         // increase matching to to match boxs
        ++Matches.at(prevFrameMatchBoxId).at(currFrameMatchBoxId);
	}
   //loop for to map bounding box in prev and current feame
    for (int columnIndex=0 ; columnIndex < prevFrame.boundingBoxes.size(); columnIndex++) {
        // get maximum element in rows ( indicate the larger matches in current frame to prev frame)
        const int rowIndex=getMaximumIndex(Matches.at(columnIndex)) ;
   
        if (Matches.at(columnIndex).at(rowIndex) == 0) 
           continue;
        //fill  output by set every box id in last frame to current frame
